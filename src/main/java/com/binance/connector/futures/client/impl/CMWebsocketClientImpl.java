...
public class CMWebsocketClientImpl extends WebsocketClientImpl
{
	 ...
	 * @return int - Connection ID
	 */
	public int indexPriceStream(String pair, int speed, WebSocketCallback onOpenCallback, WebSocketCallback onMessageCallback,
			WebSocketCallback onClosingCallback, WebSocketCallback onFailureCallback)
	{
		...
		if (speed == defaultSpeed)
		{
			request = RequestBuilder
					.buildWebsocketRequest( String.format( "%s/ws/%s@indexPrice", getBaseUrl(), pair.toLowerCase(), speed ) );
		}
		else
		{
			request = RequestBuilder
					.buildWebsocketRequest( String.format( "%s/ws/%s@indexPrice@%ss", getBaseUrl(), pair.toLowerCase(), speed ) );
		}
	...
	{
		ParameterChecker.checkParameterType( pair, String.class, "pair" );
		return markPriceSymbolsPairStream( pair, speed, getNoopCallback(), onMessageCallback, getNoopCallback(),
				getNoopCallback() );
	}
	 ...
	 * @return int - Connection ID
	 */
	public int markPriceSymbolsPairStream(String pair, int speed, WebSocketCallback onOpenCallback,
			WebSocketCallback onMessageCallback, WebSocketCallback onClosingCallback, WebSocketCallback onFailureCallback)
	{
		...
		{
			request = RequestBuilder
					.buildWebsocketRequest( String.format( "%s/ws/%s@markPrice", getBaseUrl(), pair.toLowerCase() ) );
		}
		else
		{
			request = RequestBuilder
					.buildWebsocketRequest( String.format( "%s/ws/%s@markPrice@%ss", getBaseUrl(), pair.toLowerCase(), speed ) );
		}
	...
	{
		ParameterChecker.checkParameterType( pair, String.class, "pair" );
		return indexKlineCandlestick( pair, interval, getNoopCallback(), onMessageCallback, getNoopCallback(),
				getNoopCallback() );
	}
	 ...
	 * @return int - Connection ID
	 */
	public int indexKlineCandlestick(String pair, String interval, WebSocketCallback onOpenCallback,
			WebSocketCallback onMessageCallback, WebSocketCallback onClosingCallback, WebSocketCallback onFailureCallback)
	{
		ParameterChecker.checkParameterType( pair, String.class, "pair" );
		Request request = RequestBuilder.buildWebsocketRequest(
				String.format( "%s/ws/%s@indexPriceKline_%s", getBaseUrl(), pair.toLowerCase(), interval ) );
		return createConnection( onOpenCallback, onMessageCallback, onClosingCallback, onFailureCallback, request );
	}
	...
	public int markKlineCandlestick(String symbol, String interval, WebSocketCallback onMessageCallback)
	{
		...
		return markKlineCandlestick( symbol, interval, getNoopCallback(), onMessageCallback, getNoopCallback(),
				getNoopCallback() );
	}
	 ...
	 * @return int - Connection ID
	 */
	public int markKlineCandlestick(String symbol, String interval, WebSocketCallback onOpenCallback,
			WebSocketCallback onMessageCallback, WebSocketCallback onClosingCallback, WebSocketCallback onFailureCallback)
	{
		ParameterChecker.checkParameterType( symbol, String.class, "symbol" );
		Request request = RequestBuilder.buildWebsocketRequest(
				String.format( "%s/ws/%s@markPriceKline_%s", getBaseUrl(), symbol.toLowerCase(), interval ) );
		return createConnection( onOpenCallback, onMessageCallback, onClosingCallback, onFailureCallback, request );
	}
}
