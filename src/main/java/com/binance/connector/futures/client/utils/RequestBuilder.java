...
public final class RequestBuilder
{
	...
	public static Request buildPublicRequest(String fullUrl, HttpMethod httpMethod)
	{
		try
		{
			final Request.Builder requestBuilder = new Request.Builder().addHeader( "User-Agent", USER_AGENT )
					.addHeader( "Content-Type", CONTENT_TYPE ).url( fullUrl );
			switch (httpMethod)
			{
				case POST:
	...
	public static Request buildApiKeyRequest(String fullUrl, HttpMethod httpMethod, String apiKey)
	{
		...
		{
			final Request.Builder requestBuilder = new Request.Builder().addHeader( "User-Agent", USER_AGENT )
					.addHeader( "Content-Type", CONTENT_TYPE ).addHeader( "X-MBX-APIKEY", apiKey ).url( fullUrl );
			switch (httpMethod)
			{
				case POST:
					return requestBuilder.post( RequestBody.create( "", JSON_TYPE ) ).build();
	...
	{
		System.out.println( fullUrl );// XXX
